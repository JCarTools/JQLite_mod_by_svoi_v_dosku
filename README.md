...
Лаунчер «чёрного экрана» для **JCarTools WebView**: плеер, климат, управление шторкой и быстрый запуск приложений. Работает в Android WebView через `window.androidApi`; в браузере без API доступны локальные настройки темы и акцента.
...
| Кнопка / UI | Расположение | Действие | API / команда |
| --- | --- | --- | --- |
| **Prev** `#btn-prev` | Плеер, слева | Предыдущий трек | `runEnum(TOKEN, "MEDIA_BLACK")` |
...
| Раздел | Команды |
| --- | --- |
| Плеер | `MEDIA_BLACK`, `MEDIA_PLAY`, `MEDIA_PAUSE`, `MEDIA_NEXT` |
| Шторка | `OPEN_SHTORKA`, `CLOSE_SHTORKA` |
